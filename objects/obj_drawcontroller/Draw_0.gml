	...
	if (visible && bbox_in_camera_ext(c, 32))
	{
		...
		if (!using_pattern)
		{
			if (!surface_exists(my_pal_surface))
				my_pal_surface = surface_create(sprite_get_width(spr_pal2), sprite_get_height(spr_pal2));
			if (!buffer_exists(my_pal_buffer))
			{
				...
				my_pal_buffer = buffer_create(sprite_get_width(spr_pal2) * sprite_get_height(spr_pal2) * 4, 0, 1);
