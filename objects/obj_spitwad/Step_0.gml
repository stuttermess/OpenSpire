...
if (ragereset > 0)
	ragereset--;
if (point_in_rectangle(obj_player.x, obj_player.y, x - 300, y - 60, x + 300, y + 60) && obj_player.state != states.door && obj_player.state != states.comingoutdoor)
{
	if (state != 7 && state == 3 && bombreset <= 0)
	{
		image_index = 0;
		flash = true;
		state = 7;
