for (var i = 0; i < trueYscale; i++)
{
	...
	if (!((place_meeting(check_x, check_y, obj_destructibles) || place_meeting(check_x, check_y, obj_gnomeTNTBlock) || place_meeting(check_x, check_y, obj_metalblock) || place_meeting(check_x, check_y, obj_tiledestroy)))) && place_meeting_collision(_x, _y)
	{
		...
		if (global.debugmode == 1)
		{
			...
			draw_text_color(_x, _y, string(imageIndexArray[i]), c_white, c_white, c_white, c_white, 1);
		}
	}
}
